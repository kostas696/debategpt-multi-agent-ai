...
def moderator_node(state: dict) -> dict:
    ...
    # Grab the last two AI messages (Proponent + Opponent)
    last_turn = [msg for msg in reversed(history) if isinstance(msg, AIMessage)][:2]
    if len(last_turn) < 2:
        return state  # Not enough input yet
    ...
    summary_input = (
        ...
        f"Proponent:\n{last_turn[1].content}\n\n"
        f"Opponent:\n{last_turn[0].content}\n\n"
        f"Please summarize this round fairly and detect any logical fallacies."
    )

    response = llm.invoke([HumanMessage(content=summary_input)])
    summary = response.content

    # Fallacy Detection (optional tool)
...
# Conditional router: Should the debate continue?
